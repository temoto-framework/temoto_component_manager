...
#ifndef ta_find_component_packages_TEMOTO_ACTION_H
#define ta_find_component_packages_TEMOTO_ACTION_H
...
public:
  ...
  virtual void updateParameters(const ActionParameters& parameters_in)
  {
    for (const auto& p_in : parameters_in)
    {
      ...
      else if (p_in.getType() == "cir_pointer")
      {
        param_data = boost::any_cast<temoto_component_manager::ComponentInfoRegistry*>(p_in.getData());
      }
